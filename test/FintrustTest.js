...
describe("Fintrust", () => {
  ...
  beforeEach(async () => {
    ...
    const contractFactory = await hre.ethers.getContractFactory("Fintrust");
    fintrustContract = await contractFactory.deploy();
    await fintrustContract.deployed();
    fintrustAddress = fintrustContract.address;

    [deployer, alex, rachel, kamsi, ozioma] = await hre.ethers.getSigners();

    url = hre.ethers.utils.formatBytes32String("HelloWorld");
    amount = hre.ethers.utils.parseEther("2");
  });
